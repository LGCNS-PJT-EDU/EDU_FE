      ...
      {groupedCards.map((group, groupIdx) => (
        <div key={groupIdx} className="mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 font-[pretendard]">
            {group.map((card, idx) => (
              <CardWithThumbnail key={idx} card={card} onButton1Click={onButton1Click} />
            ))}
          </div>
...
const CardWithThumbnail: React.FC<{
  card: CardItem;
  onButton1Click?: (card: CardItem) => void;
}> = ({ card, onButton1Click }) => {
  ...
  useEffect(() => {
    ...
    const youtubeThumb = getYoutubeThumbnail(card.detailUrl);
    if (youtubeThumb) {
      setThumbnail(youtubeThumb);
    } else {
      const fetchBlogThumbnail = async () => {
        try {
          const res = await fetch(`https://api.microlink.io/?url=${encodeURIComponent(card.detailUrl!)}`);
          const json = await res.json();
          setThumbnail(json?.data?.image?.url ?? null);
        } catch (e) {
      ...
      <div className="hidden h-[250px] border px-3 py-4 sm:flex bg-white rounded-[7px] overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex-col">
        <div className="h-[120px] rounded-[7px] overflow-hidden bg-gray-200">
          {thumbnail ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-gray-500">썸네일 없음</div>
          )}
        </div>
