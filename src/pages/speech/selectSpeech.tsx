...
export default function Selectspeech() {
  ...
  useEffect(() => {
    if (!getPrivacyStatus()) setShowModal(true);
  }, []);

  const handleAgree = async () => {
    try {
      await axios.post('/api/interview/privacy'); // 동의하면 api 호출
  ...
  const handleCancel = () => {
    navigate('/roadmap'); // 동의 안 하면 로드맵으로
  };
  ...
  const subjectList = tab === 'existing' ? data.existingSubjectIds : data.missingSubjectIds;
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 pt-5 pb-10 font-[pretendard]">
      ...
      <>
        {showModal && <AgreeModal onAgree={handleAgree} onClose={handleCancel} />}
        <div className="max-w-4xl mx-auto px-6 pt-5 pb-10 font-[pretendard]">
          <div className="text-sm text-[#779AF4] bg-blue-100 border border-blue-200 rounded-md p-3 mb-5">
            * 사전평가를 완료한 과목은 파란색으로 표시되며, 기본 선택 상태입니다.
          </div>
