  ...
  const query = useQuery({
    queryKey:
      nickname || email || errorCode
        ? ['admin-feedback-fail-logs', { page, nickname, email, errorCode }]
        : ['admin-feedback-fail-logs-without-filter'],
    queryFn: async () => {
      const data = await fetchFeedbackFailLogs({
        page: page - 1,
        size: 10,
        nickname,
        email,
